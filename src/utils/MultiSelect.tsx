...
export const MultiSelect: FC<ImageMultiSelectIPProps> = ({ options, placeholder, setValue, title, svalue }) => {
  const [showOptions, setShowOptions] = useState(false)
  const [filterStr, setFilterStr] = useState('')
  const [selectedLeaders, setSelectedLeaders] = useState([])

  const filteredOptions = options.filter((el: any) =>
    filterStr ? el.value.toLowerCase().includes(filterStr) : el
  )
    ...
    <>
      <section className='flex flex-col gap-1'>
        ...
        <section
          ...
          className={`flex items-center border py-1 pr-[.4rem] rounded-md gap-2 cursor-pointer relative  bg-gray-100 border-gray-200 text-gray-700 bg-gray-50 ${selectedLeaders.length > 0 ? 'pl-3' : 'pl-4'
            }`}>
          <div className='flex flex-wrap items-center gap-1 flex-1'>
            {selectedLeaders.map((el: any) => (
              <span key={el.id} className='flex capitalize py-1 px-2 w-max bg-white items-center gap-1 border border-slate-300 rounded text-[14px]'><div>
                <h6 className='font-medium capitalize'>{el.value}</h6>
              </div>
                <BiX
