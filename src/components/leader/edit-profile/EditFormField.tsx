...
export const EditFormInputField: FC<EditFormInputFieldProps> = ({
  errors,
  required,
  fieldType,
  id,
  ...
  const INPUT = {
    input: (
      <input
        ...
        className={`w-full border py-[.7rem] px-4 outline-none rounded-md text-base transition-all read-only:bg-slate-200 read-only:border-slate-300 read-only:capitalize read-only:text-slate-500 ${
          type === 'number' && 'num_inp'
        } ${
          errors[id]
