...
export const NetworksList: FC<NetworksListProps> = () => {
  ...
  return (
    <>
      <CommonBox title={`Follower's (${followers.length})`} cusJSX={[<Link href={ProtectedRoutes.followers} key={Math.random()} className='text-orange-600 text-[14px] hover:underline'>     See all   </Link>,]}>
...
const FriendBox: FC<FriendBoxProps> = ({ name, followersCount, friendImg, item }) => {
  ...
  return (
    <li className='border rounded-md overflow-hidden shadow-sm flex flex-col w-max'>
      ...
      <div className='flex flex-col items-center pt-3'>
        ...
        {item?.leaderid &&
          <p className='text-[12px] font-[500]'>{item?.followers} followers</p>
        }
      </div>
